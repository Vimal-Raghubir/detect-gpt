...
import time
from json import JSONDecodeError  # at top of file or inline
...
def get_perturbation_results(span_length=10, n_perturbations=1, n_samples=500):
    ...
    '''
    # Seeding (keeps generation deterministic)
    torch.manual_seed(0)
    np.random.seed(0)
    random.seed(0)
    ...
    # Build a metadata fingerprint for cache-compatibility
    cache_key = {
        ...
        "random_fills": args.random_fills,
        "random_fills_tokens": args.random_fills_tokens,
        "seeds": {"torch": 0, "numpy": 0, "random": 0},
    }

    # Sanitize dataset name for Windows-safe filenames
    safe_dataset_name = args.dataset.replace("/", "_").replace(":", "_")

    # File-safe stamp describing the perturbation knobs (reproducibility)
    stamp = (
        f"{safe_dataset_name}"
        f"_s{span_length}"
        f"_p{int(args.pct_words_masked*100)}"
        f"_buf{args.buffer_size}"
        f"_n{n_perturbations}"
        f"_r{n_perturbation_rounds}"
        f"_mask-{str(args.mask_filling_model_name).replace('/', '-')}"
        f"_topp{args.mask_top_p}"
        f"{'_randfill' if args.random_fills else ''}"
        f"{'_randtokens' if args.random_fills_tokens else ''}"
    )

    # Default cache paths if CLI not provided
    default_human_cache = os.path.join(PERT_HUMAN_DIR, f"{stamp}.json")
    default_ai_cache    = os.path.join(PERT_AI_DIR,    f"{stamp}.json")

    # If user passes a single --perturb_cache_in/out, we still honor it;
    # otherwise we use the default split files above.
    human_cache_in = default_human_cache
    ai_cache_in    = default_ai_cache
    human_cache_out = default_human_cache
    ai_cache_out    = default_ai_cache

    # If a single input cache is supplied, treat it as a folder (or file pair prefix) is tricky,
    # so we only override if you explicitly want to; otherwise use defaults.
    if getattr(args, "perturb_cache_in", ""):
        # If a directory passed, compose file paths; if file, assume it's the HUMAN file and AI is alongside (optional behavior).
        if os.path.isdir(args.perturb_cache_in):
            human_cache_in = os.path.join(args.perturb_cache_in, "Human", f"{stamp}.json")
            ai_cache_in    = os.path.join(args.perturb_cache_in, "AI",    f"{stamp}.json")
        else:
            human_cache_in = args.perturb_cache_in  # user-specified
            # Try an AI sibling next to a HUMAN file by name convention
            ai_cache_in = human_cache_in.replace(os.sep + "Human" + os.sep, os.sep + "AI" + os.sep)

    if getattr(args, "perturb_cache_out", ""):
        if os.path.isdir(args.perturb_cache_out):
            human_cache_out = os.path.join(args.perturb_cache_out, "Human", f"{stamp}.json")
            ai_cache_out    = os.path.join(args.perturb_cache_out, "AI",    f"{stamp}.json")
        else:
            human_cache_out = args.perturb_cache_out
            ai_cache_out    = human_cache_out.replace(os.sep + "Human" + os.sep, os.sep + "AI" + os.sep)

    # Try loading cached perturbations (expect grouped per example: list[list[str]])
    items = None
    human_loaded = ai_loaded = None
    try:
        if os.path.exists(human_cache_in):
            with open(human_cache_in, "r", encoding="utf-8") as f:
                human_loaded = json.load(f)
        if os.path.exists(ai_cache_in):
            with open(ai_cache_in, "r", encoding="utf-8") as f:
                ai_loaded = json.load(f)

        if human_loaded and ai_loaded and human_loaded.get("meta") == cache_key and ai_loaded.get("meta") == cache_key:
            ph = human_loaded.get("perturbed_original_grouped", [])
            pa = ai_loaded.get("perturbed_sampled_grouped", [])
            assert len(ph) == len(original_text) and len(pa) == len(sampled_text), "Cache length mismatch."
            items = []
            for idx in range(len(original_text)):
                items.append({
                    "original": original_text[idx],
                    "sampled":  sampled_text[idx],
                    "perturbed_original": ph[idx],  # list[str]
                    "perturbed_sampled":  pa[idx],  # list[str]
                })
            print(f"[INFO] Using cached perturbations:\n  Human: {human_cache_in}\n  AI:    {ai_cache_in}")
        else:
            if human_loaded or ai_loaded:
                print("[WARN] Perturb cache meta mismatch or only one side present; regenerating both.")
    except Exception as e:
        print(f"[WARN] Failed to read perturb caches ({e}); regenerating…")

    # Generate perturbations if no usable cache
    if items is None:
        load_mask_model()
        perturb_fn = functools.partial(perturb_texts, span_length=span_length, pct=args.pct_words_masked)

        # First round of perturbations (n_perturbations per example), returned flat
        p_sampled_text = perturb_fn([x for x in sampled_text for _ in range(n_perturbations)])
        p_original_text = perturb_fn([x for x in original_text for _ in range(n_perturbations)])

        # Additional re-perturbation rounds if requested
        for _ in range(n_perturbation_rounds - 1):
            try:
                p_sampled_text = perturb_fn(p_sampled_text)
                p_original_text = perturb_fn(p_original_text)
            except AssertionError:
                break

        assert len(p_sampled_text) == len(sampled_text) * n_perturbations, \
        ...
        # Group per example for serialization: list[list[str]]
        grouped_human = [
            p_original_text[i*n_perturbations:(i+1)*n_perturbations] for i in range(len(original_text))
        ]
        grouped_ai = [
            p_sampled_text[i*n_perturbations:(i+1)*n_perturbations] for i in range(len(sampled_text))
        ]

        # Pack items for downstream LL computation
        items = []
        for idx in range(len(original_text)):
            items.append({
                "original": original_text[idx],
                "sampled":  sampled_text[idx],
                "perturbed_original": grouped_human[idx],
                "perturbed_sampled":  grouped_ai[idx],
            })

        # Save the split caches
        try:
            with open(human_cache_out, "w", encoding="utf-8") as f:
                json.dump({"meta": cache_key, "perturbed_original_grouped": grouped_human}, f, indent=2)
            with open(ai_cache_out, "w", encoding="utf-8") as f:
                json.dump({"meta": cache_key, "perturbed_sampled_grouped": grouped_ai}, f, indent=2)
            print(f"[INFO] Saved perturbations:\n  Human: {human_cache_out}\n  AI:    {ai_cache_out}")
        except Exception as e:
            print(f"[WARN] Failed to save perturbation caches: {e}")

    # Now compute log-likelihoods with the (possibly different) base/scoring model
    load_base_model()
    results = []
    for it in tqdm.tqdm(items, desc="Computing log likelihoods"):
        # Each is list[str] (n_perturbations)
        p_sampled_ll = get_lls(it["perturbed_sampled"])
        p_original_ll = get_lls(it["perturbed_original"])
        res = {
            "original": it["original"],
            "sampled":  it["sampled"],
            "perturbed_sampled": it["perturbed_sampled"],
            "perturbed_original": it["perturbed_original"],
            "original_ll": get_ll(it["original"]),
            "sampled_ll":  get_ll(it["sampled"]),
            "all_perturbed_sampled_ll": p_sampled_ll,
            "all_perturbed_original_ll": p_original_ll,
            "perturbed_sampled_ll": float(np.mean(p_sampled_ll)),
            "perturbed_original_ll": float(np.mean(p_original_ll)),
            "perturbed_sampled_ll_std": float(np.std(p_sampled_ll)) if len(p_sampled_ll) > 1 else 1.0,
            "perturbed_original_ll_std": float(np.std(p_original_ll)) if len(p_original_ll) > 1 else 1.0,
        }
        results.append(res)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cache_dir', type=str, default=r"C:\Users\PC\.cache\huggingface\hub") # Setting explicit default cache dir
    parser.add_argument('--data_json', type=str, default="", help="Path to precomputed raw_data.json with {'original': [...], 'sampled': [...]} (optional)")    # NEW
    parser.add_argument('--perturb_cache_in', type=str, default="", help="JSON with cached perturbations to reuse (optional)")                                  # NEW
    parser.add_argument('--perturb_cache_out', type=str, default="", help="Where to save newly generated perturbations (optional)")                             # NEW
    ...
    # ---------------------------------------------------------------------------
    # ✅ Load precomputed AI-extended dataset if available, otherwise generate and save
    # ---------------------------------------------------------------------------
    DATASET_BASE = os.path.join(os.getcwd(), "Dataset")
    DATASET_AI_DIR = os.path.join(DATASET_BASE, "AI")
    PERTURB_BASE = os.path.join(DATASET_BASE, "Perturbed")
    PERT_HUMAN_DIR = os.path.join(PERTURB_BASE, "Human")
    PERT_AI_DIR = os.path.join(PERTURB_BASE, "AI")

    for d in (DATASET_BASE, DATASET_AI_DIR, PERTURB_BASE, PERT_HUMAN_DIR, PERT_AI_DIR):
        os.makedirs(d, exist_ok=True)

    # Dynamically name the AI dataset file based on the dataset argument
    safe_dataset_name = args.dataset.replace("/", "_").replace(":", "_")
    AI_DATA_PATH = os.path.join(DATASET_AI_DIR, f"AI_{safe_dataset_name}.json")

    data = None
    if args.data_json and os.path.exists(args.data_json):
        print(f"[INFO] Loading precomputed data from --data_json: {args.data_json}")
        with open(args.data_json, "r", encoding="utf-8") as f:
            data = json.load(f)
    elif os.path.exists(AI_DATA_PATH):
        print(f"[INFO] Found precomputed AI-extended dataset at {AI_DATA_PATH}")
        with open(AI_DATA_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
    else:
        print("[INFO] No precomputed dataset found. Generating new AI-extended dataset...")
        print(f'Loading dataset {args.dataset}...')
        data = generate_data(args.dataset, args.dataset_key)
        # Save generated human+AI dataset for future runs
        print(f"[INFO] Saving new dataset copy to {AI_DATA_PATH}")
        with open(AI_DATA_PATH, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2)

    # Sanity check structure
    assert isinstance(data, dict) and "original" in data and "sampled" in data, \
        "data_json must be {'original': [...], 'sampled': [...]}."
    assert len(data["original"]) == len(data["sampled"]), \
        "original and sampled must be same length."
    # ---------------------------------------------------------------------------
